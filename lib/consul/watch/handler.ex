...
defmodule Consul.Watch.Handler do
  ...
  """
  @callback handle_consul_events(events :: [Consul.Event.t()], list()) :: on_return()
  ...
  defmacro __using__(_) do
    quote do
      ...
      @doc false
      def handle_event({:consul_events, events}, state),
        do: handle_consul_events(events, state)

      @doc false
      def handle_consul_events(_events, state),
        do: {:ok, state}
